...
def load_genins_demo() -> tuple[TriangleArtifacts, pd.Series]:
    if cl is None:
        raise RuntimeError("chainladder package is not available to load demo datasets.")
    ...
    return TriangleArtifacts(incremental=incremental, cumulative=cumulative, latest_diagonal=latest_df), pd.Series(
        np.asarray(sim_ldf.values).flatten()
    )
