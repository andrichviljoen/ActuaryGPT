...
def load_genins_demo() -> tuple[TriangleArtifacts, pd.Series, str]:
    return load_chainladder_sample("genins")


def load_chainladder_sample(dataset: str = "genins") -> tuple[TriangleArtifacts, pd.Series, str]:
    dataset = (dataset or "genins").lower()
    if cl is None:
        cumulative = _load_local_genins_snapshot()
        sim_ldf = _fallback_sim_ldf(cumulative)
        source = "local_snapshot"
    else:
        tri = cl.load_sample(dataset)
...
def _load_local_genins_snapshot() -> pd.DataFrame:
    path = Path(__file__).resolve().parents[2] / "data" / "genins_sample.csv"
